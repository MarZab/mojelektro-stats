...
**HACS Home Assistant integration first.** The shipped product is `custom_components/mojelektro_stats/` — a self-contained custom component with a vendored API client under `lib/mojelektro_api/`. Users install via HACS; no PyPI package, no extra `manifest.json` requirements.

Everything else in the repo is **developer tooling**:

| Path | Role |
|------|------|
| `custom_components/mojelektro_stats/` | Integration + vendored lib — **primary** |
...
| [`docs/ai-rules/library.md`](docs/ai-rules/library.md) | Vendored `lib/mojelektro_api/` API client |
...
| Rule file | Scope |
|-----------|-------|
| [`home-assistant.md`](docs/ai-rules/home-assistant.md) | `custom_components/mojelektro_stats/` (integration) |
| [`library.md`](docs/ai-rules/library.md) | `custom_components/mojelektro_stats/lib/mojelektro_api/` |
...
7. **Version truth:** `custom_components/mojelektro_stats/lib/mojelektro_api/__about__.py` + matching `manifest.json` `"version"`. Bump both by hand, in lockstep.
8. **Reading-type catalog** is hardcoded in `lib/mojelektro_api/reading_types.py`. Regenerate with `make regen-reading-types`.
